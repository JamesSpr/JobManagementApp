...
def resize_columns(sheet, widths, col_offset=0, row_offset=0):
    for i, width in enumerate(widths):
        sheet.column_dimensions[chr(65+i+col_offset)].width = width + 2
...
class GenerateFinancialReport(graphene.Mutation):
    ...
    def mutate(self, root, info, date_range):
        ...
        # Data Filters
        approved_estimates = Estimate.objects.filter(approval_date__isnull=False, job_id__stage="APP", approval_date__gte=date_range[0], approval_date__lt=date_range[1])
        underway_estimates = Estimate.objects.filter(approval_date__isnull=False, job_id__stage="UND", approval_date__gte=date_range[0], approval_date__lt=date_range[1])
        invoicing1_estimates = Estimate.objects.filter(approval_date__isnull=False, job_id__stage="INV", approval_date__gte=date_range[0], approval_date__lt=date_range[1])
        invoicing2_estimates = Estimate.objects.filter(approval_date__isnull=False, job_id__stage="BSA", approval_date__gte=date_range[0], approval_date__lt=date_range[1])
        awaiting_payment_estimates = Estimate.objects.filter(approval_date__isnull=False, job_id__stage="PAY", approval_date__gte=date_range[0], approval_date__lt=date_range[1])
        finalised_estimates = Estimate.objects.filter(approval_date__isnull=False, job_id__stage="FIN", approval_date__gte=date_range[0], approval_date__lt=date_range[1])
        ...
        approved_headers = ["Client", "Job Number", "Location", "Building", "Title", "Approval Date", "Approved Price"]
        approved_types = ["General", "General", "General", "General", "General", "dd/mm/yyyy", "$#,##0.00"]
        approved_total_row = ["Total", f"=COUNTA({approved_table_name}[{approved_headers[1]}])", "", "", "", "", f"=SUBTOTAL(109,{approved_table_name}[{approved_headers[6]}])"]
        ...
        underway_headers = ["Client","Job Number","Location","Building","Title", "Approval Date","Approved Price","Start Date","Finish Date (EST)"]
        underway_types = ["General", "General", "General", "General", "General", "dd/mm/yyyy", "$#,##0.00", "dd/mm/yyyy" ]
        underway_total_row = ["Total", f"=COUNTA({underway_table_name}[{underway_headers[1]}])", "", "", "", "", f"=SUBTOTAL(109,{underway_table_name}[{underway_headers[6]}])", ""]
        ...
        invoicing_headers = ["Client","Job Number","Location","Building","Title", "Status", "Approval Date","Approved Price","Close Out Date","Invoice #","Inv Created Date","Days Since Progress"]
        invoicing_types = ["General", "General", "General", "General", "General", "General", "dd/mm/yyyy", "$#,##0.00", "dd/mm/yyyy", "General", "dd/mm/yyyy", "General"]
        invoicing_total_row = ["Total", f"=COUNTA({invoicing_table_name}[{invoicing_headers[1]}])", "", "", "", "", "", f"=SUBTOTAL(109,{invoicing_table_name}[{invoicing_headers[7]}])", "", "", "", f"=SUBTOTAL(101,{invoicing_table_name}[{invoicing_headers[11]}])"]
        ...
        for est in invoicing2_estimates:
            ...
            for invoice in inv:
                for val_i, val in enumerate(invoicing2_values(job, est, invoice)):
                    ...
                    invoicing[f"{chr(65 + val_i)}{len(invoicing1_estimates)+2}"].number_format = invoicing_types[val_i]
        ...
        awaiting_payment_headers = ["Client", "Job Number", "Location", "Building", "Title", "Amount", "Invoice #", "Invoice Sent", "Days Since Submission"]
        awaiting_payment_types = ["General", "General", "General", "General", "General", "$#,##0.00", "General", "dd/mm/yyyy", "General"]
        awaiting_payment_total_row = ["Total", f"=COUNTA({awaiting_payment_table_name}[{awaiting_payment_headers[1]}])", "", "", "", f"=SUBTOTAL(109,{awaiting_payment_table_name}[{awaiting_payment_headers[5]}])", "", "", f"=SUBTOTAL(101,{awaiting_payment_table_name}[{awaiting_payment_headers[8]}])"]
        ...
        for est in awaiting_payment_estimates:
            ...
            for invoice in inv:
                for val_i, val in enumerate(awaiting_payment_values(job, est, invoice)):
                    ...
                    awaiting_payment[f"{chr(65 + val_i)}{i+2}"].number_format = awaiting_payment_types[val_i]
        ...
        finalised_headers = ["Client", "Job Number", "Location", "Building", "Title", "Amount", "Invoice #", "Date Paid", "Days to Pay"]
        finalised_types = ["General", "General", "General", "General", "General", "$#,##0.00", "General", "dd/mm/yyyy", "General"]
        finalised_total_row = ["Total", f"=COUNTA({finalised_table_name}[{finalised_headers[1]}])", "", "", "", f"=SUBTOTAL(109,{finalised_table_name}[{finalised_headers[5]}])", "", "", f"=SUBTOTAL(101,{finalised_table_name}[{finalised_headers[8]}])"]
        finalised_col_widths = [len(x) for x in finalised_headers]

        def finalised_values(job, est, inv): 
        ...
        for est in finalised_estimates:
            job = est.job_id
            inv = Invoice.objects.filter(job=job)
            for invoice in inv:
                for val_i, val in enumerate(finalised_values(job, est, invoice)):
                    finalised[f"{chr(65 + val_i)}{finalised_estimates_counter+2}"] = val
                    finalised[f"{chr(65 + val_i)}{i+2}"].number_format = finalised_types[val_i]
        ...
        summary_headers = ["Job Status", "Total Jobs", "Total Amount"]
        summary_types = ["General", "General", "$#,##0.00"]
        summary_total_row = ["Total WIP", f"=SUBTOTAL(109,{summary_table_name}[{summary_headers[1]}])", f"=SUBTOTAL(109,{summary_table_name}[{summary_headers[2]}])"]
        ...
        for i in range(6, 10):
            summary[f"D{i}"].number_format = "$#,##0.00"
        ...
        summary["D12"].number_format = "$#,##0.00"
        summary["D13"].number_format = "$#,##0.00"

        summary.column_dimensions["B"].width = 32
        summary.column_dimensions["C"].width = 10.82
        summary.column_dimensions["D"].width = 15.73

        summary["B3"] = f"As of {now.strftime('%d/%m/%Y HH:MM')} for the period {date_range[0].strftime('%d/%m/%Y')} - {date_range[1].strftime('%d/%m/%Y')}"
